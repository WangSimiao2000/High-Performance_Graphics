...
#undef SHADERDIR_
	...
	//pipeline layout
	lut::PipelineLayout create_pipeline_layout(lut::VulkanContext const&, VkDescriptorSetLayout aSceneDescriptorLayout, VkDescriptorSetLayout aObjectDescriptorLayout);
	...
	void create_postprocessing_framebuffers(
	...
	void record_commands(
		...
		VkDescriptorSet,
		std::vector<TriangleMesh> const& aModleMesh,
...
int main() try
{
	...
	lut::Pipeline pipe_A = create_gbuffer_pipeline(window, renderPassA.handle, pipeLayout.handle);
	// create gbuffer
	...
	create_gbuffer_framebuffers(window, renderPassA.handle, gbuffer_framebuffer, gbufferAttachments);
	...
	lut::Pipeline pipe_B = create_postprocessing_pipeline(window, renderPassB.handle, postprocessingPipeLayout.handle);
	create_postprocessing_framebuffers(window, renderPassB.handle, framebuffers, gbufferAttachments);

	lut::CommandPool cpool = lut::create_command_pool(window, VK_COMMAND_POOL_CREATE_TRANSIENT_BIT | VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT);

	std::vector<VkCommandBuffer> cbuffersA;
	std::vector<VkCommandBuffer> cbuffersB;
	std::vector<lut::Fence> cbfencesA;
	std::vector<lut::Fence> cbfencesB;

	for (std::size_t i = 0; i < framebuffers.size(); ++i)
	{
		cbuffersA.emplace_back(lut::alloc_command_buffer(window, cpool.handle));
		cbuffersB.emplace_back(lut::alloc_command_buffer(window, cpool.handle));
		cbfencesA.emplace_back(lut::create_fence(window, VK_FENCE_CREATE_SIGNALED_BIT));
	...
	TriangleMesh bakedMesh;

	std::vector<TriangleMesh> modelMesh;
	...
	for (auto const& mesh : bakedObject.meshes)
	{
		...
		bakedMesh = prepareMeshes(window, allocator, positions, texcoords, normals, indices);
	...
	std::size_t j = 0;
	for (std::size_t i = 0; i < bakedObject.textures.size() - 2; i += 4)
	{
		auto materialId = bakedObject.meshes[j].materialId * 4;
		...
		texViews.push_back(lut::create_image_view_texture2d(window, texImages[j].image, VK_FORMAT_R8G8B8A8_SRGB));
		...
		mtlViews.push_back(lut::create_image_view_texture2d(window, mtlImages[j].image, VK_FORMAT_R8G8B8A8_SRGB));
		...
		rogViews.push_back(lut::create_image_view_texture2d(window, rogImages[j].image, VK_FORMAT_R8G8B8A8_SRGB));

		VkWriteDescriptorSet desc[3]{};
		VkDescriptorImageInfo textureInfo{};
		textureInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
		textureInfo.imageView = texViews[j].handle;
		...
		mtlTexInfo.imageView = mtlViews[j].handle;
		...
		rogTexInfo.imageView = rogViews[j].handle;
	...
	while (!glfwWindowShouldClose(window.window)) {
		...
		if (recreateSwapchain)
		{
			...
			if (changes.changedSize)
			{
				pipe_A = create_gbuffer_pipeline(window, renderPassA.handle, pipeLayout.handle);
				pipe_B = create_postprocessing_pipeline(window, renderPassB.handle, postprocessingPipeLayout.handle);
			}
		...
		assert(std::size_t(imageIndex) < cbfencesA.size());
		if (auto const res = vkWaitForFences(window.device, 1, &cbfencesA[imageIndex].handle, VK_TRUE, std::numeric_limits<std::uint64_t>::max()); VK_SUCCESS != res)
		{
			throw lut::Error("Unable to wait for command buffer fence %u\n"
				"vkWaitForFences() returned %s", imageIndex, lut::to_string(res).c_str());
		}
		if (auto const res = vkResetFences(window.device, 1, &cbfencesA[imageIndex].handle); VK_SUCCESS != res)
		{
		...
		assert(std::size_t(imageIndex) < cbuffersA.size());
		...
		submit_commands(
			...
			cbuffersA[imageIndex],
			cbuffersB[imageIndex],
			cbfencesA[imageIndex].handle,
			cbfencesB[imageIndex].handle,
			imageAvailable.handle,
			renderFinished_Gbuffer.handle,
			renderFinished_Postprocessing.handle
		);

		present_results(window.presentQueue, window.swapchain, imageIndex, renderFinished_Postprocessing.handle, recreateSwapchain);
...
namespace {
	...
	void record_commands(
		...
		std::vector<TriangleMesh> const& aModelMesh,
		std::vector<VkDescriptorSet> const& aDescriptorA
	)
	{
	...
	void submit_commands(lut::VulkanWindow const& aWindow, VkCommandBuffer aCmdBuff, VkCommandBuffer aCmdBuffB, VkFence aFenceA, VkFence aFenceB, VkSemaphore aWaitSemaphore, VkSemaphore aSignalSemaphore, VkSemaphore aSignalSemaphoreB)
	{
		vkResetFences(aWindow.device, 1, &aFenceA);
		...
		if (auto const res = vkQueueSubmit(aWindow.graphicsQueue, 1, &submitInfo, aFenceA); VK_SUCCESS != res)
		{
