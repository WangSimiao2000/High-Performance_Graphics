...
int main() try
{
	...
	BakedModel bakedObject = load_baked_model("assets/cw2/suntemple.comp5822mesh");
	...
	for (auto const& mesh : bakedObject.meshes)
	{
		...
		bakedMesh = prepareMesh(window, allocator, positions, texcoords, normals, indices);
	...
	std::size_t j = 0;
	for (std::size_t i = 0;i < bakedObject.textures.size(); i += 4)
	{
		int materialId = bakedObject.meshes[j].materialId * 4;
		const auto& texInfo = bakedObject.textures[materialId];
		...
		const auto& mtlInfo = bakedObject.textures[materialId + 2];
		...
		const auto& rogInfo = bakedObject.textures[materialId + 1];
		...
		const auto& norInfo = bakedObject.textures[materialId + 3];
		std::string norTexturePath = norInfo.path;

		norImages.push_back(lut::load_image_texture2d(norTexturePath.c_str(), window, loadCmdPool_nor.handle, allocator));
		norViews.push_back(lut::create_image_view_texture2d(window, norImages[j].image, VK_FORMAT_R8G8B8A8_SRGB));

		VkDescriptorImageInfo norTexInfo{};
		norTexInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
		norTexInfo.imageView = norViews[j].handle;
...
namespace {
	...
	lut::DescriptorSetLayout create_object_descriptor_layout(lut::VulkanWindow const& aWindow)
	{
		...
		VkDescriptorSetLayoutCreateInfo layoutInfo{};
		layoutInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO;
		layoutInfo.bindingCount = 4;// sizeof(bindings) / sizeof(bindings[0]);
		layoutInfo.pBindings = bindings;
		VkDescriptorSetLayout layout = VK_NULL_HANDLE;
	...
	lut::Pipeline create_pipeline(lut::VulkanWindow const& aWindow, VkRenderPass aRenderPass, VkPipelineLayout aPipelineLayout)
	{
		...
		stages[1].pName = "main";
//#if CURRENT_STATE == MESH_DENSITY
//		stages[2].sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
//		stages[2].stage = VK_SHADER_STAGE_GEOMETRY_BIT;
//		stages[2].module = geom.handle;
//		stages[2].pName = "main";
//#endif // CURRENT_STATE == MESH_DENSITY
